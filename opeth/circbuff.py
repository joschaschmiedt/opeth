...
class CircularBuffer(Sequence):
    ...
    def append(self, value):
        ...
        if len(self) + appendcnt > self._capacity:
            raise BufferError(
                "Append: ring buffer is full, cap: %d, result would be: %d+%d = %d"
        ...
        if self._right_index + elemcnt > self._arr.shape[self._append_axis]:
            ...
            if self._append_axis == 0:
                self._arr[0 : self._right_index - self._left_index] = self._arr[
    ...
    def drop(self, nof_elements):
        ...
        if len(self) >= nof_elements:
            ...
            raise BufferError(
                "Attempt to drop %d items but only %d present"
                % (nof_elements, len(self))
            )
    ...
    def _adjust_index(self, idx, leftid, rightid):
        ...
        if upd < self._left_index or upd >= self._right_index:
            raise IndexError(
                "Invalid index received: %d modified to %d on axis %d"
    ...
    def __getitem__(self, item):
        ...
        if type(upd) == int:
            ...
            slice_start = (
                self._left_index
                if upd.start is None
                else self._adjust_index(upd.start, self._left_index, self._right_index)
            )
            slice_stop = (
                self._right_index
                if upd.stop is None
                else self._adjust_index(upd.stop, self._left_index, self._right_index)
            )
