...
class SpikeEvalGui(object):
    ...
    def __init__(self, sample_rate):
        ...
        self.spikeplot = self.spikeplotwidget_raw.plot(
            [0], [0], pen=pg.mkPen(width=self.PENWIDTH), antialias=True
        )
        self.spikeplot_thresh = self.spikeplotwidget_raw.plot(
            [0], [0], pen=pg.mkPen(width=self.PENWIDTH), antialias=False
        )
        ...
        for i in range(self.NOF_SPIKEPLOTS):
            ...
            self.spikeplotpositions.append(
                self.spikeplotpool[-1].plot(
                    [0], [0], symbolBrush=(255, 0, 0), symbolPen="w"
                )
            )
        ...
        self.par_update_on_spikes = Parameter.create(
            name="Update only on spikes", type="bool", value=True
        )
    ...
    def plot(self, data_ts, data, spike_ts, spike_pos, threshold_levels):
        ...
        for ts, pos in zip(spike_ts[ch], spike_pos[ch]):
            ...
            self.spikeplotpositions[self.spikeplotpool_next].setData(
                [data_ts[pos]], [data[ch][pos]]
            )
            self.spikeplotcurves[self.spikeplotpool_next].setData(
                data_ts[plotrange],
                data[ch][plotrange],
                pen=pg.mkPen(
                    color=self.COLORS[self.spikeplotpool_next],
                    width=self.PENWIDTH,
                    antialias=True,
                ),
            )
            self.spikerawcurves[self.spikeplotpool_next].setData(
                data_ts[plotrange],
                data[ch][plotrange],
                pen=pg.mkPen(
                    color=self.COLORS[self.spikeplotpool_next],
            ...
            self.spikeplotpool_next = (self.spikeplotpool_next + 1) % len(
                self.spikeplotpool
            )
