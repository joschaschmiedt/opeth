...
from colldata import EVENT_ROI, SAMPLES_PER_SEC, SPIKE_HOLDOFF, DataProc
from comm import CommProcess
from debug import TimeMeasClass  # used for DEBUG_TIMING
...
DEFAULT_SPIKE_THRESHOLD = (
    0.00003  #: Spike threshold set as default parameter if no ini file found.
)
...
MAX_CHANNELS_PER_PLOT = (
    8  #: Maximal number of channels for a given histogram window/polytrode
)
...
CHANNELPLOTS_ANTIALIASED = (
    True  #: More professional display for per channel plots, but less visible
)
...
class GuiClass(object):
    ...
    * Spike analysis windows using :class:`spike_gui.SpikeEvalGui` when corresponding button is pressed.
    ...
    def __init__(self):
        ...
        self.should_restore_params = (
            True  #: Automatic parameter reload should happen only on startup.
        )
        ...
        self.earliest_hist_plot = (
            default_timer()
        )  #: Next histogram update time for performance cap

        self.timing_start = (
            default_timer()
        )  #: Debug: internal elapsed time measurement scheduler
        ...
        self.disabled_channel_update_to = (
            ""  # doing that instead from the update routine
        )
    ...
    def update_channelcnt(self, nChannels):
        ...
        self.nPlotWindows = int(
            math.ceil(nChannels / self.channels_per_plot)
        )  #: Number of histogram plots, equals to ``self.nChannels/self.channels_per_plot``
        logger.info(
            "Initializing for %d channel plots, %d histogram plots"
            % (nChannels, self.nPlotWindows)
        )
    ...
    def init_rawwin(self):
        ...
        self.rawdatawin = pg.GraphicsLayoutWidget(title="Raw analog samples")
    ...
    def init_histwin(self):
        ...
        msg += (
            '<tr><th align="left">Spike censoring time (SPIKE_HOLDOFF):</th><td style="padding-left:1em">%.2f ms</td></tr>'
            % (SPIKE_HOLDOFF * 1000)
        )
        msg += (
            '<tr><th align="left">Histogram bin size (HISTOGRAM_BINSIZE):</th><td style="padding-left:1em">%.2f ms</td></tr>'
            % (HISTOGRAM_BINSIZE * 1000)
        )
        msg += (
            '<tr><th align="left">Inverted spike detection (NEGATIVE_THRESHOLD):</th><td style="padding-left:1em">%s</td></tr>'
            % ("yes" if NEGATIVE_THRESHOLD else "no")
        )
        msg += "</table>"
    ...
    def populate_histwin(self):
        ...
        self.docks = [
            Dock("%s %d" % (self.plottitle, idx + 1))
            for idx in range(self.nPlotWindows)
        ]
        ...
        cols = (
            min(
                np.argwhere(
                    np.array(COLUMN_INCREMENTS, dtype="int32") >= self.nPlotWindows
                )
            )[0]
            + 1
        )
        ...
        for row in range(1, maxrow + 1):
            ...
            self.histplotarea.addDock(
                self.docks[todock], "bottom", self.docks[docked_above]
            )
        ...
        for row in range(0, maxrow + 1):
            for col in range(1, maxcol + 1):
                ...
                self.histplotarea.addDock(
                    self.docks[todock], "right", self.docks[docked_to_left]
                )

        self.histwidgets = [
            pg.PlotWidget(viewBox=pgext.DisabledMouseViewBox())
            for i in range(self.nPlotWindows)
        ]
        ...
        for idx, (d, w) in enumerate(zip(self.docks, self.histwidgets)):
            ...
            for i in range(self.channels_per_plot):
                ...
                color = (
                    self.display_brushcolors[i]
                    if ch_id not in self.disabled_channels
                    else (255, 255, 255, 0)
                )  # transparent
                hp = w.plot(
                    np.arange(2), np.arange(1), stepMode=True, fillLevel=0, brush=color
                )
            ...
            histplots.append(
                w.plot(
                    ...
                    stepMode=True,
                    fillLevel=0,
                    brush=self.display_brushcolors[0],
                )
            )
        ...
        self.hist_x = np.linspace(
            ...
            int(
                round(
                    (self.event_roi[1] - self.event_roi[0] + HISTOGRAM_BINSIZE)
                    / HISTOGRAM_BINSIZE
                )
            ),
        )
    ...
    def update_plotcolors(self):
        ...
        for ch_id in range(self.nChannels):
            color = (
                self.display_linecolors[ch_id % self.channels_per_plot]
                if ch_id not in self.disabled_channels
                else (255, 255, 255, 0)
            )
    ...
    def init_params(self, paramcontainer, reset=False, **kwargs):
        ...
        self.par_sampling_rate = Parameter.create(
            name="Sampling rate",
            type="int",
        ...
        self.par_ttl_src = Parameter.create(
            name="Event trigger channel", type="int", limits=(1, MAX_TRIGGER_CHANNEL)
        )
        self.param.addChild(self.par_ttl_src)

        self.par_ch_per_plot = Parameter.create(
            name="Channels per plot",
            type="int",
            limits=(1, MAX_CHANNELS_PER_PLOT),
            value=CHANNELS_PER_HISTPLOT,
        )
        self.param.addChild(self.par_ch_per_plot)

        self.par_disabled_ch = Parameter.create(
            name="Disabled channels", type="str", value=""
        )
        self.param.addChild(self.par_disabled_ch)

        self.par_ttlroi_before = Parameter.create(
            name="ROI before event",
            type="float",
            value=self.event_roi[0],
            step=1e-3,
            siPrefix=True,
            limits=(-500, 500),
            suffix="s",
        ...
        self.par_ttlroi_after = Parameter.create(
            name="ROI after event",
            type="float",
            value=self.event_roi[1],
            step=1e-3,
            siPrefix=True,
            limits=(-500, 500),
            suffix="s",
        ...
        self.par_histcolor = Parameter.create(
            name="Histogram color",
            type="list",
            values=dict([(p, p) for p in PLOT_TYPES]),
        )
        self.param.addChild(self.par_histcolor)

        self.par_common_thresh = Parameter.create(
        ...
        self.param_custom_thresh = Parameter.create(
            name="Custom threshold levels",
            type="group",
            children=self.par_tetrode_thresh,
        )
        ...
        row1.setLayout(h1)
        self.configbox = QtWidgets.QGroupBox("Config:")
        self.configbox.setLayout(h2)

        save_params_btn = QtWidgets.QPushButton("Save")
        save_params_btn.setMinimumWidth(5)
        save_as_params_btn = QtWidgets.QPushButton("Save as")
        save_as_params_btn.setMinimumWidth(5)
        load_params_btn = QtWidgets.QPushButton("Load")
        load_params_btn.setMinimumWidth(5)
        reset_params_btn = QtWidgets.QPushButton("Reset")
        reset_params_btn.setMinimumWidth(5)
        clear_btn = QtWidgets.QPushButton("Clear plot")
        change_theme_btn = QtWidgets.QPushButton("Invert colours")

        open_spikes_btn = QtWidgets.QPushButton("Open new spike win")
    ...
    def populate_params(self):
        ...
        if (
            self.threshold_levels is not None
            and len(self.threshold_levels) == self.nChannels
        ):
            thresholds = list(
                (self.threshold_levels / VOLT_TO_UVOLT_MULTIPLIER).ravel()
            )
        else:
        ...
        if self.channels_per_plot == 1:
            ...
            channel_names = [
                "Ch#%d (%s#%d):"
                % (ch + 1, self.plottitle, ch // self.channels_per_plot + 1)
    ...
    def init_debugwin(self):
        ...
        self.debug_ttlcnt = self.debug_param.addChild(
            Parameter.create(name="TTL count", type="int", value=0)
        )
        self.debug_datamax = self.debug_param.addChild(
            Parameter.create(name="Data max", type="float", value=1e9)
        )
        self.debug_datamin = self.debug_param.addChild(
            Parameter.create(name="Data min", type="float", value=-1e9)
        )
        self.debug_trigdatamax = self.debug_param.addChild(
            Parameter.create(name="Trig. data max", type="float", value=1e9)
        )
        self.debug_trigdatamin = self.debug_param.addChild(
            Parameter.create(name="Trig. data min", type="float", value=-1e9)
        )
        self.debug_channelcnt = self.debug_param.addChild(
            Parameter.create(name="Channel cnt", type="int", value=self.nChannels)
        )
        t = ParameterTree()
    ...
    def update_threshold_levels(self):
        ...
        if hasattr(self, "par_tetrode_thresh") and hasattr(self, "nChannels"):
            ...
            self.threshold_levels = (
                levels.reshape(self.nChannels, 1) * VOLT_TO_UVOLT_MULTIPLIER
            )
    ...
    def update_disabled_channels(self):
        ...
        if self.par_disabled_ch.value() != distr2:
            self.disabled_channel_update_at = default_timer() + 0.1
    ...
    def change_event_roi(self, new_roi, clear_plot=True, **kwargs):
        ...
        self.ttl_range_ms = int(
            round((self.event_roi[1] - self.event_roi[0]) / HISTOGRAM_BINSIZE)
        )
    ...
    def load_params(self):
        ...
        if cfg.has_option("processing", "spike_nthreshold_channels"):
            ...
            for par, thr in zip(
                self.par_tetrode_thresh[:common_size], thr[:common_size]
            ):
    ...
    def clear_plot(self):
        ...
        self.spike_bin_ms = np.zeros(
            (self.cp.collector.channel_cnt(), self.ttl_range_ms + 1)
        )
        self.force_update = True
    ...
    def update_histograms(self):
        ...
        * per channel: lines instead of bar graphs to make it possible to
        ...
        spike_bin_ms_disabled[self.disabled_channels] = np.zeros(
            (len(self.disabled_channels), spike_bin_ms_disabled.shape[1])
        )
        ...
        for plot_idx, histplot in enumerate(self.histplots):
            if self.par_histcolor.value() == PLOT_AGGREGATE:
                ...
                for i in range(self.channels_per_plot):
                    sum_spikes = np.sum(
                        spike_bin_ms_disabled[startid : startid + i + 1], axis=0
                    )
                ...
                sum_spikes = np.sum(
                    spike_bin_ms_disabled[
                        plot_idx
                        * self.channels_per_plot : (plot_idx + 1)
                        * self.channels_per_plot
                    ],
                    axis=0,
                )
                ...
                for i in range(self.channels_per_plot)[::-1]:
                    ...
                    histplot[self.channels_per_plot - 1 - i].setData(
                        self.hist_x * 1000, [0] * (len(self.hist_x) - 1)
                    )
        ...
        for plot_idx, channelplot in enumerate(self.channelplots):
            ...
            if self.par_histcolor.value() == PLOT_CHANNELS:
                ...
                for i in range(self.channels_per_plot - 1, -1, -1):
                    channelplot[i].setData(
                        self.hist_x,
                        spike_bin_ms_disabled[startid + i]
                        + (self.channels_per_plot - i - 1)
                        * CHANNELPLOTS_VERTICAL_OFFSET,
                    )
            else:
    ...
    def update_spikewins(self, data_ts, data, spike_ts, spike_pos):
        ...
        for spikewin in self.spikewins:
            if NEGATIVE_THRESHOLD:
                spikewin.plot(
                    data_ts, data, spike_ts, spike_pos, -self.threshold_levels
                )
            else:
    ...
    def update(self, **kwargs):
        ...
        * update spike analysis windows :meth:`update_spikewins`
        ...
        if (
            self.disabled_channel_update_at is not None
            and self.disabled_channel_update_at < default_timer()
        ):
        ...
        if DEBUG_FPS and (
            default_timer() - self.last_fps_report > DEBUG_FPSREPORT_PERIOD
        ):
            fps = (self.framecnt - self.lastframecnt) / (
                default_timer() - self.last_fps_report
            )
            self.fpslist.append(fps)

            logger.info(
                "\r%.1f updates / sec [%.1f - %.1f], compr: %.3f @ %.1f"
                % (
                    fps,
                    min(self.fpslist),
                    max(self.fpslist),
        ...
        if self.initiated and (new_samprate != self.sampling_rate):
            logger.info(
                "Detected sampling rate change: %d -> %d"
                % (self.sampling_rate, new_samprate)
            )
        ...
        for i in range(len(self.rawdata_curves)):
            self.rawdata_curves[i].setData(
                tscomp, datacomp[i] - 1.5 * i * self.plotdistance
            )
        self.timeas.toc("04-curves")

        # In case there's no trigger from open ephys we can simulate it
        thresh_levels = (
            -self.threshold_levels if NEGATIVE_THRESHOLD else self.threshold_levels
        )
        if AUTOTRIGGER_CH is not None:
            # hack: inject TTL events (no actual HW to generate TTL, can't make OE properly play back TTLs)
            ttl = self.dataproc.autottl(
                data,
                ts,
                self.cp.collector.timestamp,
                ch=AUTOTRIGGER_CH,
                threshold=thresh_levels[0],
            )
        ...
        while 1:
            ...
            data_at_ttl, data_ts = self.cp.collector.process_ttl(
                ttl_ch=self.par_ttl_src.value() - 1,
            ...
            data_ts = data_ts / float(
                self.sampling_rate
            )  # adjusted to seconds (instead of sample index)

            data_ts_0 = (
                data_ts - data_ts[0]
            )  # timestamps starting at 0 for first sample
            data_ts_roi = (
                data_ts_0 + self.event_roi[0]
            )  # and to the TTL window (-20ms..+50ms)

            self.timeas.tic("06-spikedetect")
            # todo: check whether spike_ts can be used instead of data_ts[spike_pos[ch]]

            spike_pos, spike_ts = self.dataproc.spikedetect(
                data_at_ttl,
                data_ts_roi,
        ...
        if was_new_data or self.force_update:
            ...
            if self.earliest_rawttl_plot < default_timer():
                # display updates
                self.ttlplot.setTitle(
                    "Samples around event #%d (@%.3f)"
                    % (self.displayed_ttlcnt, default_timer() - self.starttime)
                )

                self.earliest_rawttl_plot = (
                    default_timer() + 0.5
                ...
                for i in range(len(self.ttlraw_curves)):
                    if data_ts_roi is not None:
                        self.ttlraw_curves[i].setData(
                            data_ts_roi * 1000,
                            last_data_at_ttl[i] - 1.5 * i * self.plotdistance,
                        )

            if self.spikewins:
                if data_ts_roi is not None:
                    self.update_spikewins(
                        data_ts_roi, last_data_at_ttl, spike_ts, spike_pos
                    )
