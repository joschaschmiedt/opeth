...
class Collector(object):
    ...
    def __init__(self):
        ...
        self.set_sampling_rate(
            1
        )  # will be overridden when first data packet is received
    ...
    def update_channels(self, channels):
        ...
        if channels != self.channels:
            if self.channels == 0:
                ...
                logger.error(
                    "Channel count changed from %d to %d, please restart OPETH"
                    % (self.channels, channels)
                )
        ...
        **kwargs
        ...
        while 1:
            ...
            if ttl_ch is not None and ttl.event_channel != ttl_ch:
                ...
                logger.info(
                    "Dropping TTL timestamp %d - last data ts: %d"
                    % (ttl.timestamp, self.tsbuffer[-1])
                )
                self.ttls.popleft()
                continue
            else:
            ...
            if (
                tsrange_min < self.tsbuffer[0]
            ):  # corresponding data is already lost, drop this TTL
                logger.info(
                    "TTL timestamp %d earlier than available data %d, skipping"
                    % (tsrange_min, self.tsbuffer[0])
                )
                self.ttls.popleft()
                continue
    ...
    def set_sampling_rate(self, sampling_rate):
        ...
        self.timestamp_per_sec = (
            sampling_rate  # current open ephys report timestamps as sample index
        )
        self.max_data_amount = (
            2 * self.timestamp_per_sec
        )  #: Buffering limit (sample count)
...
class DataProc(object):
    ...
    def compress(self, data, rate, timestamps=None):
        ...
        if timestamps is not None:
            ...
            tsmin, tsmax = datats.min(axis=1).reshape(-1, 1), datats.max(
                axis=1
            ).reshape(-1, 1)
        ...
        for i in range(data.shape[0]):
            ...
            mins, maxes = dproc.min(axis=1).reshape(-1, 1), dproc.max(axis=1).reshape(
                -1, 1
            )

            compressed[i, :] = np.concatenate(
                (mins, maxes), axis=1
            ).ravel()  # a row of min0, max0, min1, max1... for each "rate" sized data chunk
