...
class TimeMeasClass(object):
    ...
    def __init__(self):
        ...
        self.timestart = defaultdict(
            int
        )  #: Last measurement's start time (initialized in :meth:`tic`)
        self.timecount = defaultdict(
            int
        )  #: Number of measurements collected in :attr:`timespent` (for averaging)
    ...
    def dump(self, logger):
        ...
        for i in sorted(self.timestart.keys()):
            logger.debug(
                "%15s: %f / %d" % (str(i), self.timespent[i], self.timecount[i])
            )
