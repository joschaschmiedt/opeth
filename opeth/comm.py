...
class CommProcess:
    ...
    def send_event(
        self, event_list=None, event_type=3, sample_num=0, event_id=2, event_channel=1
    ):
        """
        Note:
        ...
        * Processes incoming events.
        ...
        if self.isTesting:
            if np.random.random() < 0.005:
                self.send_event(
                    event_type=3, sample_num=0, event_id=self.event_no, event_channel=1
                )

        start = default_timer()
        timeout = (
            start + COMMPROCESS_MAX_POLLTIME
        )  # spend maximum this amount of time in the loop
        ...
        while default_timer() < timeout:
            if (time.time() - self.last_heartbeat_time) > 2.0:
                ...
                if self.socket_waits_reply:
                    logger.error(
                        "No reply to heartbeat, retrying... (Don't panic. :) )"
                    )
                    ...
                    if (time.time() - self.last_reply_time) > 10.0:
                        ...
                        logger.warning(
                            "Looks like we lost the server, trying to reconnect"
                        )
                    ...
                    if self.isStats:  # statistics
                        ...
                        if self.msgstat_start + 1 < default_timer():
                            ...
                            sizes, counts = np.unique(
                                self.msgstat_size, return_counts=True
                            )
                            logger.debug(sizes, counts)
                            logger.debug(
                                sum(counts), "messages", sum(self.msgstat_size), "bytes"
                            )
                            self.msgstat_size = []
                            self.msgstat_start = default_timer()
